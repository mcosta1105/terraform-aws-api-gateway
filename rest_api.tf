...
resource "aws_api_gateway_method" "rest_method" {
  ...
  authorization = (each.value.authorization != null && each.value.authorization == false) ? "NONE" : (var.enable_custom_authorizer ? "CUSTOM" : "NONE")
  authorizer_id = (each.value.authorization != null && each.value.authorization == false) ? null : (var.enable_custom_authorizer ? aws_api_gateway_authorizer.rest_authorizer[var.custom_authorizers[0].name].id : null)
